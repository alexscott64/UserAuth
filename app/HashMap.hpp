...
private:
    // This is our hash table which is a dynamically allocated array of buckets.
    // If we have just one * for Node (ie. Node*) we end up having our linked
    // list, which will hold the actual data (user/pw)
    Node** hashTable;
    ...
    unsigned int sz;
    //int *keysPerBucket; // holds the amount of keys we have in each
    //void createNode(Node* nodeValue, const std::string& key, const std::string& value);
